...
def get_match_outcome(name, match_id):
    participants = get_match_details(match_id)
...
def get_bot_lane_deaths(name, match_id):
    participants = get_match_details(match_id)
    player_team = None
    bot_lane_deaths = 0
    for player in participants:
    ...
    if player_team is not None:
        bot_lane_deaths = sum(player['deaths'] for player in participants 
                              if player['lane'] == 'BOTTOM' and player['teamId'] == player_team)
    return bot_lane_deaths

# returns the average number of bot lane deaths in the player's team from the last 20 games
def calc_avg_bot_lane_deaths(name, match_history):
    if not match_history:
        return 0
    total_bot_lane_deaths = sum(get_bot_lane_deaths(name, match) for match in match_history)
    avg_bot_lane_deaths = total_bot_lane_deaths / len(match_history)
...
def get_stats():
    ...
    for name, summoner_id in summoners:
        try:
            ...
            avg_bot_lane_deaths = calc_avg_bot_lane_deaths(name, match_history)
            player_stats = {
                'name': name,
                'aka': config['aka'][name],
